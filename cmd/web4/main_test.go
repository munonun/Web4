...
func TestGossipHello1RateLimitedBeforeVerify(t *testing.T) {
	cases := []struct {
		...
		{name: "suite0_default", disableSuite: "0"},
		{name: "suite1_legacy", disableSuite: "1"},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("WEB4_HANDSHAKE_DISABLE_SUITE0", tc.disableSuite)
			...
			selfD, err := node.NewNode(filepath.Join(homeD, ".web4mvp"), node.Options{})
			if err != nil {
				t.Fatalf("load node D failed: %v", err)
			}
