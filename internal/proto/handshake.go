...
func DecodeHello1Fields(m Hello1Msg) ([32]byte, [32]byte, []byte, []byte, []byte, error) {
	var fromID, toID [32]byte
	fromBytes, err := hex.DecodeString(m.FromNodeID)
	if err != nil || len(fromBytes) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad from_node_id")
	}
	toBytes, err := hex.DecodeString(m.ToNodeID)
	if err != nil || len(toBytes) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad to_node_id")
	}
	ea, err := hex.DecodeString(m.EA)
	if err != nil || len(ea) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad ea")
	}
	na, err := hex.DecodeString(m.Na)
	if err != nil || len(na) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad na")
	}
	sig, err := hex.DecodeString(m.Sig)
	if err != nil || len(sig) == 0 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad sig")
	}
	copy(fromID[:], fromBytes)
	copy(toID[:], toBytes)
	return fromID, toID, ea, na, sig, nil
}

func DecodeHello2Fields(m Hello2Msg) ([32]byte, [32]byte, []byte, []byte, []byte, error) {
	var fromID, toID [32]byte
	fromBytes, err := hex.DecodeString(m.FromNodeID)
	if err != nil || len(fromBytes) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad from_node_id")
	...
	if err != nil || len(eb) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad eb")
	}
	nb, err := hex.DecodeString(m.Nb)
	if err != nil || len(nb) != 32 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad nb")
	}
	sig, err := hex.DecodeString(m.Sig)
	if err != nil || len(sig) == 0 {
		return fromID, toID, nil, nil, nil, fmt.Errorf("bad sig")
	...
	copy(fromID[:], fromBytes)
	copy(toID[:], toBytes)
	return fromID, toID, eb, nb, sig, nil
}
